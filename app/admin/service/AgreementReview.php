...
class AgreementReview extends BaseService
{
    public function index($param){
        $page = $param['page'] ? $param['page'] : 1;
        $limit = $param['limit'] ? $param['limit'] : 10;
        $where = [];
        if(array_key_exists('agreement_id', $param)){
            $where[] = ['agreement_id',"=",$param['agreement_id']];
        }
    ...
    public function add($agreement_id, $type, $title, $video, $date,$files){
        Db::startTrans();
        try {
            if($type == 1){
